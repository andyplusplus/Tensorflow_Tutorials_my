...
model.compile(optimizer=optimizer, loss='categorical_crossentropy', metrics=['accuracy']) # In[13]: # For a classification-problem such as MNIST which has 10 possible classes, we need to use the loss-function called `categorical_crossentropy`. The performance metric we are interested in is the classification accuracy.
...
result = model.evaluate(x=data.x_test, y=data.y_test) # In[15]: # ### Evaluation # Now that the model has been trained we can test its performance on the test-set. This also uses numpy-arrays as input.
...
if True: # In[23]: # ### Examples of Mis-Classified Images
...
result = model2.evaluate(x=data.x_test, y=data.y_test) # In[31]: # ### Evaluation
...
layer_conv1   # <tensorflow.python.keras.layers.convolutional.Conv2D at 0x7fab0c052d30>
...
image1 = data.x_test[0] # In[59]: # Plot an image from the test-set which will be used as an example below.
plot_image(image1)
